...
console.log("total number of filims :-",filims.length);

// movie names
var filmNames=filims.map(f=>f.title)

console.log("film names :-",filmNames);

// movies whose runtime below 140
var runtimefilter=filims.filter(f=>f.runtime<140).map(f=>f.title)   

console.log("runtime filter = ",runtimefilter);

// movie with highest run time
var highestRunTime=filims.reduce((m1,m2)=>Number(m1.runtime)>Number(m2.runtime)?m1:m2)
console.log("highest run time : ",highestRunTime);

// movie with lowest run time
var lowestRunTime=filims.reduce((f1,f2)=>Number(f1.runtime)<Number(f2.runtime)?f1:f2)

console.log("lowest run time : ",lowestRunTime);
...
var actionmovies=filims.filter(f=>f.genres.includes ("Action")).map(f=>f.title)
console.log("actionmovies = ",actionmovies);

// movie with genre either Action or Drama
var actionmovies=filims.filter(f=>f.genres.includes ("Action" || "Drama")).map(f=>f.title)
console.log("action or drama = ",actionmovies);

// year in which most number of movies released
console.log("year in which most number of movies released",Object.entries(yc).reduce((o1,o2)=>o1[1]>o2[1]?o1:o2));